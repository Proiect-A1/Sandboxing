...
public:
    ...
    void insert(std::string submission_id, std::string problem_id, int rev_id); /// aduaga submisia in map; PE VIITOR TREBUIE BAGATA SI IN problem_manager
    ...
    void set_test_result(std::string submission_id, int test_id, submission_test test_result); /// marcheaza ca s-a terminat de evaluat un test
    submission_data get_submission(std::string submission_id); /// returneaza structura cu informatiile despre submisie, inclusiv rezultatele testelor
};
