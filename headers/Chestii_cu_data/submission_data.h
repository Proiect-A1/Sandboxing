...
struct submission_data{
    ...
    std::string upload_link;
    
    std::vector<submission_test> tests;
    std::vector<submission_group> groups;
    submission_data(){}
    submission_data(std::string submission_data , language_enum language, std::string problem_id, int rev_id, std::string download_link , std::string upload_link , int socket_fd=1);
