...
void ezsolve()
{
   ...
   {
      vector<vector<NrMare>> dp(2, vector<NrMare>(n+1, NrMare{}));
      vvi tat(n+1, vi(n+1));
      dp[0][1] = NrMare{1};
      for (int i = 1; i <= n; ++i)
      {
         ...
         for (int j = 1; j <= n; ++j)
            ...
            {
               if (compara(dp[i-1&1][j], dp[i&1][j-1]) > 0)
                  dp[i&1][j] = dp[i-1&1][j],
                  inmulteste(dp[i&1][j], mat[i][j]),
                  tat[i][j] = 1;
               else
                  dp[i&1][j] = dp[i&1][j-1],
                  inmulteste(dp[i&1][j], mat[i][j]),
      ...
      if (!dp[n&1][n].empty() && imparte(dp[n&1][n], numi))
      {
         inmulteste(dp[n&1][n], numr);
